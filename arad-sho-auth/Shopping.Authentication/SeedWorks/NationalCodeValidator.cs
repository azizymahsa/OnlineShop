...
namespace Shopping.Authentication.SeedWorks
{
    public static class NationalCodeValidator
    {
        ...
        public static Boolean IsValidNationalCode(this string nationalCode)
        {
            ...
            if (nationalCode.Length != 10)
                throw new CustomException("طول کد ملی باید ده کاراکتر باشد");

            //در صورتی که کد ملی ده رقم عددی نباشد
            var regex = new Regex(@"\d{10}");
            if (!regex.IsMatch(nationalCode))
                throw new CustomException("کد ملی تشکیل شده از ده رقم عددی می‌باشد؛ لطفا کد ملی را صحیح وارد نمایید");
